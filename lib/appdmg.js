...
module.exports = exports = function (options) {
  ...
  pipeline.addStep('Creating temporary image', function (next) {
    ...
    {
      var tmpobj = tmp.dirSync({ mode: 0750, prefix: 'dmg_' });
      global.temporaryImagePath = tmpobj.name;
      global.temporaryMountPath = global.temporaryImagePath;
      next(null);
    }
  ...
  pipeline.addStep('Finalizing image', function (next) {
    if (os.platform() == 'darwin')
      hdiutil.create(global.opts.title, global.temporaryImagePath, global.target, next);
    else
      hdiutil.createAlternative(global.temporaryImagePath, global.target, next);
  });
